...
import org.openhab.binding.insteon.handler.InsteonBridgeHandler;
import org.openhab.binding.insteon.handler.InsteonHandler;
...
public class InsteonHandlerFactory extends BaseThingHandlerFactory {
    ...
    private static final Set<ThingTypeUID> SUPPORTED_THING_TYPES_UIDS = Sets.union(
            InsteonBindingConstants.SUPPORTED_DEVICE_THING_TYPES_UIDS,
            InsteonBindingConstants.SUPPORTED_BRIDGE_THING_TYPES_UIDS);
    ...
    protected @Nullable ThingHandler createHandler(Thing thing) {
        ...
        if (InsteonBindingConstants.SUPPORTED_BRIDGE_THING_TYPES_UIDS.contains(thingTypeUID)) {
            InsteonBridgeHandler handler = new InsteonBridgeHandler((Bridge) thing);
            return handler;
        } else if (supportsThingType(thingTypeUID)) {
            return new InsteonHandler(thing);
        }
        return null;
    }
}
